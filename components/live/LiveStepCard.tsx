...
const URGENCY_STYLES: Record<UrgencyLevel, string> = {
  /* allow-arbitrary: bg-white/[0.035] — non-subpanel urgency-state tint, no canonical token */
  normal: "border-white/[0.08] bg-white/[0.035]",
  attention: "border-orange-300/45 bg-orange-500/[0.07]",
  /* allow-arbitrary: shadow-[...] critical-urgency glow — no canonical ds.shadow.* tier */
...
const ZONE_RAIL_ACTIVE: Record<LiveZone, string> = {
  /* allow-arbitrary: shadow-[...] direct-zone active glow — no canonical ds.shadow.* tier */
...
const ZONE_DOT_ACTIVE: Record<LiveZone, string> = {
  /* allow-arbitrary: shadow-[...] direct-zone dot glow — no canonical ds.shadow.* tier */
...
export default function LiveStepCard({
  ...
  return (
    <div className="space-y-2">
      ...
      <section
        /* allow-arbitrary: rounded-[1.35rem] — step card radius, no canonical ds.radius.* tier */
      ...
      >
        ...
        <div className="flex min-w-0 flex-wrap items-center gap-1.5">
          {zoneRailItems.map(({ key, label }) => {
            ...
            return (
              <span
                key={key}
                className={`inline-flex max-w-full items-center gap-1.5 rounded-full border px-2.5 py-1 ${ds.text.body9} font-black uppercase tracking-[0.13em] transition-all duration-300 ${
                  isActive
                    ? ZONE_RAIL_ACTIVE[key]
                    : `border-white/[0.07] bg-transparent ${ds.color.mutedClass.disabled}`
                }`}
              >
                <span
          ...
          {currentStep.tempTarget !== null && (
            /* allow-arbitrary: bg-white/[0.05] — non-subpanel temp chip surface, no canonical token */
            <span className={`ml-auto shrink-0 rounded-full border border-white/10 bg-white/[0.05] px-2.5 py-1 ${ds.text.body11} font-black ${ds.color.mutedClass.secondary}`}>
              {currentStep.tempTarget}°C
            </span>
          )}
        </div>

        <p className={`mt-3 ${ds.text.body10} font-black uppercase tracking-[0.22em] ${ds.color.mutedClass.disabled}`}>
          {text.currentStep}
        </p>
        {/* allow-arbitrary: text-[clamp(...)] display-tier step heading — stays inline per slice-d-tokens.md §1 */}
        ...
        {/* allow-arbitrary: text-[clamp(...)] responsive body — stays inline per slice-d-tokens.md §1 */}
        <p className={`mt-2.5 overflow-hidden whitespace-pre-line text-[clamp(0.9rem,3.5vw,1rem)] font-semibold leading-snug ${ds.color.mutedClass.base} [display:-webkit-box] [-webkit-box-orient:vertical] [-webkit-line-clamp:4]`}>
          {currentStep.instructions}
        </p>

        {checkCopy && (
          <div className="mt-3 flex items-start gap-2 rounded-xl border border-yellow-300/[0.14] bg-yellow-400/[0.05] px-3 py-2">
            <span className={`mt-px shrink-0 ${ds.text.body11} leading-none text-yellow-300/70`}>⚠</span>
            <div className="min-w-0">
              <p className={`${ds.text.body9} font-black uppercase tracking-[0.16em] text-yellow-200/45`}>
                {text.checkBeforeNext}
              </p>
              <p className={`mt-0.5 ${ds.text.body11} font-semibold leading-snug text-yellow-200/70`}>
                {checkCopy}
              </p>
            </div>
          </div>
